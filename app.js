...
<div class="summaryCard"><span>مجموع الرصيد</span><b class="${s.players.reduce((sum,p)=>sum+(b[p]?.balance||0),0)<0?'negText':s.players.reduce((sum,p)=>sum+(b[p]?.balance||0),0)>0?'posText':''}">${moneyBlank(s.players.reduce((sum,p)=>sum+(b[p]?.balance||0),0))}</b></div>
...
<span class="${clsMoney(d.amount)} depoAmountGroup"><b>${depositTypeLabel(d)}</b>&nbsp;&nbsp;${money(d.amount)}</span>
</div>`).join('');teamsMatchSelect.innerHTML=s.matches.sort((a,b)=>b.date.localeCompare(a.date)).map(m=>`<option value="${m.id}">${formatDateDisplay(m.date)}  ${m.place||m.location||''}</option>`).join('');renderTempGuests();renderCalendar();renderCalendarList();renderMatchLog(s);renderTeams();renderTables(s,b);updatePrettyDates()}
