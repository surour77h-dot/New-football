...
let currentPage='home', calendarView=new Date(), selectedCalendarDate='', tempGuests=[], tempTeamMap={}, adjustEditorDraft=null, activeDateTarget='';
...
function state(){let s;try{s=JSON.parse(localStorage.getItem(LS)||'{}')}catch(e){s={}};s.players=s.players||[];s.matches=s.matches||[];s.deposits=s.deposits||[];s.teams=s.teams||{};s.extraCharges=s.extraCharges||[];s.extraDiscounts=s.extraDiscounts||[];s.settings=s.settings||{};return s}
...
function goPage(id){
 currentPage=id||'home';document.body.classList.toggle('settings-page',currentPage==='settings');
 document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===currentPage));document.querySelectorAll('[data-page]').forEach(b=>b.classList.toggle('active',b.dataset.page===currentPage));
 const titleBox=document.getElementById('pageTitle');if(titleBox)titleBox.textContent=pageTitle(currentPage);
 if(typeof pagesMenu!=='undefined'&&pagesMenu)pagesMenu.classList.remove('open');
 try{renderAll()}catch(e){console.error(e)}
 document.querySelectorAll('.page').forEach(p=>p.classList.toggle('active',p.id===currentPage));document.body.classList.toggle('settings-page',currentPage==='settings');if(titleBox)titleBox.textContent=pageTitle(currentPage);
}
...
function isInactiveThisYear(last){return !last||new Date(last).getFullYear()<new Date().getFullYear()}
function refreshSelects(s){const opts=s.players.map(p=>`<option>${escapeHtml(p)}</option>`).join('');guestOwner.innerHTML=opts;depositPlayer.innerHTML=opts;editPlayerSelect.innerHTML=opts;playerFilterSelect.innerHTML='<option value="">اختر لاعب</option>'+opts}
...
function renamePlayer(){const s=state(),old=editPlayerSelect.value,n=editPlayerName.value.trim();if(!old||!n)return;s.players=s.players.map(p=>p===old?n:p);s.matches.forEach(m=>{m.players=(m.players||[]).map(p=>p===old?n:p);(m.guests||[]).forEach(g=>{if(g.owner===old)g.owner=n})});s.deposits.forEach(d=>{if(d.player===old)d.player=n});editPlayerName.value='';save(s)}
function deletePlayer(){
  const s=state(),p=editPlayerSelect.value;if(!p||!confirm('حذف '+p+'؟'))return;
  s.players=s.players.filter(x=>x!==p);s.matches.forEach(m=>{m.players=(m.players||[]).filter(x=>x!==p);m.guests=(m.guests||[]).filter(g=>g.owner!==p)});s.deposits=s.deposits.filter(d=>d.player!==p);
  saveNoRender(s);renderAll();showConfirm(`تم حذف اللاعب ${escapeHtml(p)} من ضمن قروب الكورة`,'playerTable');
}
function addGuest(){const guest=guestName.value.trim(),owner=guestOwner.value;if(!guest||!owner)return;tempGuests.push({guest,owner});guestName.value='';renderTempGuests();renderMatchPreview()}
function saveMatch(){
  const s=state();const id=editingMatchId.value||uid(),date=matchDate.value||today(),placeVal=document.getElementById('place').value||'',booking=Number(bookingCost.value||0),need=Number(neededPlayers.value||0),price=need?booking/need:0,players=[...document.querySelectorAll('.playerCheck:checked')].map(x=>x.value);
  const row={id,date,place:placeVal,bookingCost:booking,neededPlayers:need,price,players,guests:[...tempGuests]};
...
function saveDeposit(){
  ...
  editingDepositId.value='';depositAmount.value='';saveNoRender(s);renderAll();showConfirm(opMessage(type,amt,player),'deposits');
}
function clearDeposit(){editingDepositId.value='';depositAmount.value=''}
function renderDeposits(s){
  const rows=[...s.deposits].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(d=>{let type=d.type;const raw=String(d.date||'').replace(/-/g,'/');if(!type&&(raw==='2026/01/01'||raw==='1/1/2026'))type='initial';if(!type)type='in';const cls=(type==='out')?'moneyNeg':(type==='late'?'moneyLate':'moneyPos'),sign=(type==='out'||type==='late')?'-':'';return `<div class="tRow" onclick="openDepositEditor('${d.id}')"><span>${escapeHtml(d.player||'')}</span><span>${fmDate(d.date)}</span><span class="${cls}">${depositTypeLabel({...d,type})} ${sign}${money(Math.abs(d.amount||0))}</span></div>`}).join('');
...
function openDepositEditor(id){const s=state(),d=s.deposits.find(x=>x.id===id);if(!d)return;editDepositModalId.value=d.id;editDepositModalPlayer.innerHTML=s.players.map(p=>`<option>${escapeHtml(p)}</option>`).join('');editDepositModalPlayer.value=d.player;editDepositModalDate.value=d.date||today();editDepositModalAmount.value=Math.abs(d.amount||0);let type=d.type;const raw=String(d.date||'').replace(/-/g,'/');if(!type&&(raw==='2026/01/01'||raw==='1/1/2026'))type='initial';editDepositModalType.value=type||'in';updateDateButtons();depositEditModal.classList.add('show')}
function closeDepositEditor(){depositEditModal.classList.remove('show');goPage('deposits');scrollToDepositsTop()}
function saveDepositEditor(){
  const s=state(),id=editDepositModalId.value,d=s.deposits.find(x=>x.id===id);if(!d)return;
  d.player=editDepositModalPlayer.value;d.date=editDepositModalDate.value||today();d.amount=Math.abs(Number(editDepositModalAmount.value||0));d.type=editDepositModalType.value||'in';saveNoRender(s);depositEditModal.classList.remove('show');renderAll();showConfirm(opMessage(d.type,d.amount,d.player),'deposits');scrollToDepositsTop();
}
function deleteDepositEditor(){
...
function teamHtml(s,m){
  ...
  const guestSet=(m.guests||[]).map(g=>`${g.guest} (${g.owner})`);
  const noMembers=no.filter(n=>!guestSet.includes(n)),noGuests=no.filter(n=>guestSet.includes(n));
...
function renderPlayerReport(){
  ...
  const depRows=deps.map(d=>{let type=d.type;const raw=String(d.date||'').replace(/-/g,'/');if(!type&&(raw==='2026/01/01'||raw==='1/1/2026'))type='initial';if(!type)type='in';const sign=(type==='out'||type==='late')?'-':'',cls=(type==='out')?'moneyNeg':(type==='late'?'moneyLate':'moneyPos');return `<div class="tRow"><span>${fmDate(d.date)}</span><span>${depositTypeLabel({...d,type})}</span><span class="${cls}">${sign}${money(Math.abs(d.amount||0))}</span></div>`}).join('');
...
function renderCalendar(){
 const s=state(),y=calendarView.getFullYear(),mo=calendarView.getMonth(),first=new Date(y,mo,1),last=new Date(y,mo+1,0),monthMs=s.matches.filter(m=>{let d=new Date(m.date);return d.getFullYear()===y&&d.getMonth()===mo});
 calendarMonthTitle.textContent=calendarView.toLocaleDateString('en-US',{month:'long',year:'numeric'}).toUpperCase();
...
function saveExtraUnified(){
  ...
  saveNoRender(s);renderAll();showConfirm(`تم اضافة مبلغ ${money(Math.abs(amount))} بنجاح`,'accounts');
}
...
function renderPageOrder(){
  ...
  box.innerHTML=`<div class="newPageOrder">${order.map(id=>`<div class="pageOrderRow"><span>${pageTitle(id)}</span><div><button type="button" onclick="movePage('${id}',-1);event.preventDefault();return false;">↑</button><button type="button" onclick="movePage('${id}',1);event.preventDefault();return false;">↓</button></div></div>`).join('')}</div>`;
}
function movePage(id,dir){
  const order=getPageOrder(),i=order.indexOf(id),j=i+dir;
  if(i<0||j<0||j>=order.length)return false;
  [order[i],order[j]]=[order[j],order[i]];
  savePageOrder(order);currentPage='settings';renderAll();goPage('settings');return false;
}
...
function showConfirm(message,targetPage){
  ...
  msg.innerHTML=message;modal.classList.add('show');
  btn.onclick=function(){modal.classList.remove('show');if(targetPage)goPage(targetPage);};
}
function opMessage(type,amount,player){
  const a=money(Math.abs(Number(amount||0)));
  if(type==='initial')return `تم ايداع ${a} مبدئيا لـ${escapeHtml(player)} بنجاح`;
  if(type==='out')return `تم خصم ${a} من ${escapeHtml(player)} بنجاح`;
  if(type==='late')return `تم تسجيل تأخير وخصم ${a} من ${escapeHtml(player)} بنجاح`;
  return `تم ايداع ${a} لـ${escapeHtml(player)} بنجاح`;
}
function scrollToDepositsTop(){setTimeout(()=>{document.getElementById('deposits')?.scrollIntoView({block:'start'});},80);}

/* ===== R8 final behavior fixes ===== */
function showConfirm(message,targetPage){const modal=document.getElementById('confirmModal'),msg=document.getElementById('confirmMessage'),btn=document.getElementById('confirmCloseBtn');if(!modal||!msg||!btn){if(targetPage)goPage(targetPage);return}msg.innerHTML=message;modal.classList.add('show');btn.onclick=function(){modal.classList.remove('show');if(targetPage)goPage(targetPage)}}
...
function saveDepositEditor(){const s=state(),id=editDepositModalId.value,d=s.deposits.find(x=>x.id===id);if(!d)return;d.player=editDepositModalPlayer.value;d.date=editDepositModalDate.value||today();d.amount=Math.abs(Number(editDepositModalAmount.value||0));d.type=editDepositModalType.value||'in';localStorage.setItem(LS,JSON.stringify(s));depositEditModal.classList.remove('show');renderAll();goPage('deposits');setTimeout(()=>document.getElementById('deposits')?.scrollIntoView({block:'start'}),50)}
function deleteDepositEditor(){const s=state(),id=editDepositModalId.value;if(!id)return;s.deposits=s.deposits.filter(d=>d.id!==id);localStorage.setItem(LS,JSON.stringify(s));depositEditModal.classList.remove('show');renderAll();goPage('deposits');setTimeout(()=>document.getElementById('deposits')?.scrollIntoView({block:'start'}),50)}
...
function applyDatePicker(){
  ...
  closeDatePicker();updateDateButtons();
...
/* === v15 REAL FINAL: force operation colors + amounts formatting everywhere === */
(function(){
  const GREEN_CLASS='moneyPos', RED_CLASS='moneyNeg', ORANGE_CLASS='moneyLate';
  function v15Money(n){ return Number(n||0).toFixed(3); }
  function v15IsZero(n){ return Math.abs(Number(n||0)) < 0.0005; }
  function v15Kind(d){
    let t=String((d&& (d.type||d.kind||d.operation||d.label))||'').trim().toLowerCase();
    let ar=String((d&& (d.operation||d.label||d.typeName))||'').trim();
    const raw=String((d&&d.date)||'').replace(/-/g,'/');
    if(!t && (raw==='2026/01/01'||raw==='1/1/2026')) return 'initial';
    ...
    if(t==='initial' || ar.includes('مبدئي')) return 'initial';
    return 'deposit';
  }
  function v15Class(kind){
    if(kind==='late') return ORANGE_CLASS;
    if(kind==='debt'||kind==='discount'||kind==='out') return RED_CLASS;
    return GREEN_CLASS;
  }
  function v15Label(kind){
    if(kind==='late') return 'تأخير';
    if(kind==='debt') return 'مديونية';
    if(kind==='discount') return 'خصم';
    if(kind==='initial') return 'إيداع.م';
  ...
  function v15Amount(amount, kind){
    const n=Number(amount||0);
    if(v15IsZero(n)) return '';
    const txt=v15Money(Math.abs(n));
    if(kind==='late'||kind==='debt'||kind==='discount'||kind==='out'||n<0) return txt+'-';
    return txt;
  }
  function v15Signed(n){
    const v=Number(n||0);
    if(v15IsZero(v)) return '';
    return v15Money(Math.abs(v))+(v<0?'-':'');
  }
  function v15BalanceClass(n, kind){
    if(kind) return v15Class(kind);
    n=Number(n||0); return n<0?RED_CLASS:GREEN_CLASS;
  }
  window.amountTextByKind=v15Amount;
  window.signedAmountText=v15Signed;
  window.opKind=v15Kind;
  window.opClsFromKind=v15Class;
  window.depositShortLabelByKind=v15Label;
  window.fmAmount=v15Signed;
  window.amountClass=v15BalanceClass;

  window.renderDeposits = renderDeposits = function(s){
    const rows=[...((s&&s.deposits)||[])].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(d=>{
      const kind=v15Kind(d), cls=v15Class(kind), label=v15Label(kind), amt=v15Amount(d.amount,kind);
      const text=amt ? `${label} ${amt}` : label;
      return `<div class="tRow" onclick="openDepositEditor('${escAttr(d.id||'')}')"><span>${escapeHtml(d.player||'')}</span><span>${fmDate(d.date)}</span><span class="${cls}">${text}</span></div>`;
    }).join('');
    depositsList.innerHTML=`<div class="compactTable depositsMoneyTable"><div class="tHead"><span>اللاعب</span><span>التاريخ</span><span class="amountHead">المبلغ</span></div>${rows||'<p class="muted">لا توجد عمليات</p>'}</div>`;
  };

  window.renderPlayerReport = renderPlayerReport = function(){
    const s=state(), p=playerFilterSelect.value;
    if(!p){ playerFilterContent.innerHTML=''; return; }
    const b=balances(s)[p]||{}, deps=(s.deposits||[]).filter(d=>d.player===p), games=(s.matches||[]).filter(m=>(m.players||[]).includes(p));
    const playPlusDebt=(b.playTotal||0)+(b.debtDeposits||0);
    const gameRows=games.map(g=>`<div class="tRow"><span>${fmDate(g.date)}</span><span>${escapeHtml(g.place||'')}</span><span class="moneyNeg">${v15Amount(g.price||0,'debt')}</span></div>`).join('');
    const depRows=deps.map(d=>{
      const kind=v15Kind(d), cls=v15Class(kind), label=v15Label(kind), amt=v15Amount(d.amount,kind);
      return `<div class="tRow"><span>${fmDate(d.date)}</span><span class="${cls}">${label}</span><span class="${cls}">${amt}</span></div>`;
    }).join('');
    playerFilterContent.innerHTML=`<div class="reportStats"><div class="statBox depositStat"><span>الإيداعات</span><b class="moneyPos">${v15Amount(b.deposits,'deposit')}</b></div><div class="statBox gamesStat"><span>اللعب</span><b>${b.games||''}</b></div><div class="statBox playTotalStat"><span>إجمالي اللعب + المديونية</span><b class="moneyNeg">${v15Amount(playPlusDebt,'debt')}</b></div><div class="statBox lateStat"><span>التأخير</span><b class="moneyLate">${v15Amount(b.late,'late')}</b></div><div class="statBox balanceStat"><span>الرصيد</span><b class="${v15BalanceClass(b.balance)}">${v15Signed(b.balance)}</b></div></div><div class="card"><h3>أيام اللعب</h3><div class="compactTable reportTable"><div class="tHead"><span>التاريخ</span><span>المكان</span><span class="amountHead">المبلغ</span></div>${gameRows||'<p class="muted">لا يوجد</p>'}</div></div><div class="card"><h3>الإيداعات والمديونيات</h3><div class="compactTable reportTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${depRows||'<p class="muted">لا يوجد</p>'}</div></div>`;
  };

  window.renderAccounts = renderAccounts = function(s){
    const b=balances(s), neg=(s.players||[]).filter(p=>(b[p]?.balance||0)<0), late=(s.deposits||[]).filter(d=>v15Kind(d)==='late');
    const extra=(s.extraCharges||[]).reduce((a,x)=>a+Number(x.amount||0),0), discount=(s.extraDiscounts||[]).reduce((a,x)=>a+Number(x.amount||0),0);
    const lateTotal=late.reduce((a,d)=>a+Math.abs(Number(d.amount||0)),0), debt=neg.reduce((a,p)=>a+Math.abs(b[p].balance),0)+discount, final=extra+lateTotal-debt;
    const debtRows=neg.map(p=>`<div class="tRow" onclick="openPlayerReport('${escAttr(p)}')"><span>${escapeHtml(p)}</span><span>${fmDate(b[p].last)}</span><span class="moneyNeg">${v15Amount(Math.abs(b[p].balance),'debt')}</span></div>`).join('');
    const lateRows=late.map(d=>`<div class="tRow" onclick="openPlayerReport('${escAttr(d.player)}')"><span>${escapeHtml(d.player)}</span><span>${fmDate(d.date)}</span><span class="moneyLate">${v15Amount(d.amount,'late')}</span></div>`).join('');
    accountsContent.innerHTML=`<div class="accountMiniCards"><div><span>المديونية</span><b class="moneyNeg">${v15Amount(debt,'debt')}</b></div><div><span>التأخير</span><b class="moneyLate">${v15Amount(lateTotal,'late')}</b></div><div><span>الإضافي</span><b class="moneyPos">${v15Amount(extra,'deposit')}</b></div><div><span>الإجمالي</span><b class="${v15BalanceClass(final)}">${v15Signed(final)}</b></div></div>
    <div class="card"><h3>اللاعبين المدانين والمتأخرين</h3><div class="compactTable accountsMoneyTable"><div class="tHead"><span>الاسم</span><span>التاريخ</span><span class="amountHead">المبلغ</span></div>${debtRows}${lateRows}${(!debtRows&&!lateRows)?'<p class="muted">لا يوجد</p>':''}</div></div>
    <div class="card"><div class="sectionTitleLine"><b>سجل الخصم / الإضافة</b><button onclick="openAdjustEditor()">تعديل</button></div>${renderAdjustRows(s)}</div>
    <div class="card"><div class="sectionTitleLine"><b>خصم / إضافة</b></div><div class="grid2 accountsExtraGrid"><label>النوع<select id="extraType"><option value="extra">إضافة</option><option value="discount">خصم</option></select></label><label>التاريخ<button id="extraDateBtn" class="fakeDateInput" type="button" onclick="openDatePicker('extraDate')">${fmDate(today())}</button><input id="extraDate" type="hidden" value="${today()}"></label><label>المبلغ<input id="extraAmount" type="number" step="0.001"></label><label>الملاحظة<input id="extraNote"></label></div><button class="primary wide saveBtn" onclick="saveExtraUnified()">حفظ</button></div>`;
    if(typeof updateDateButtons==='function') updateDateButtons();
  ...
  window.renderAdjustRows = renderAdjustRows = function(s){
    ...
    return `<div class="compactTable accountsMoneyTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${rows.map(r=>{const cls=v15Class(r.kind);return `<div class="tRow"><span>${fmDate(r.date)}</span><span class="${cls}">${r.t}</span><span class="${cls}">${v15Amount(r.amount,r.kind)}</span></div>`}).join('')||'<p class="muted">لا يوجد</p>'}</div>`;
  };

  setTimeout(()=>{ try{ renderAll(); }catch(e){} },50);
})();

/* === v16 HARD APPLY: unified player-table colors/fonts + amount rules === */
(function(){
  const EPS=0.0005;
  function m(n){ return Number(n||0).toFixed(3); }
  function zero(n){ return Math.abs(Number(n||0)) < EPS; }
  function typeOf(d){
    const t=String((d&&(d.type||d.kind||d.operation))||'').toLowerCase();
    const ar=String((d&&(d.operation||d.label||d.typeName))||'');
    const raw=String((d&&d.date)||'').replace(/-/g,'/');
    if(t==='late'||ar.includes('تأخير')) return 'late';
    if(t==='out'||t==='debt'||ar.includes('مديونية')) return 'debt';
    if(t==='discount'||ar.includes('خصم')) return 'discount';
  ...
  function cls(k){ return k==='late'?'moneyLate':(k==='debt'||k==='discount'||k==='out'?'moneyNeg':'moneyPos'); }
  function label(k){ return k==='late'?'تأخير':k==='debt'?'مديونية':k==='discount'?'خصم':k==='initial'?'إيداع.م':'إيداع'; }
  function amt(n,k){ if(zero(n)) return ''; const s=m(Math.abs(Number(n||0))); return (k==='late'||k==='debt'||k==='discount'||k==='out'||Number(n)<0) ? s+'-' : s; }
  function signed(n){ if(zero(n)) return ''; const v=Number(n||0); return m(Math.abs(v))+(v<0?'-':''); }
  function balClass(n){ return Number(n||0)<0?'moneyNeg':'moneyPos'; }
  function setClass(el,k){ if(!el) return; el.classList.remove('moneyPos','moneyNeg','moneyLate'); el.classList.add(cls(k)); }
  window.depositTypeLabel=function(d){ return label(typeOf(d)); };
  window.fmAmount=signed; window.amountClass=balClass;

  window.renderDeposits = renderDeposits = function(s){
    const rows=[...((s&&s.deposits)||[])].sort((a,b)=>(b.date||'').localeCompare(a.date||'')).map(d=>{
      const k=typeOf(d), c=cls(k), a=amt(d.amount,k); const txt=a?`${label(k)} ${a}`:label(k);
      return `<div class="tRow" onclick="openDepositEditor('${escAttr(d.id||'')}')"><span>${escapeHtml(d.player||'')}</span><span>${fmDate(d.date)}</span><span class="${c}">${txt}</span></div>`;
    }).join('');
    depositsList.innerHTML=`<div class="compactTable depositsMoneyTable unifiedMoneyTable"><div class="tHead"><span>اللاعب</span><span>التاريخ</span><span class="amountHead">المبلغ</span></div>${rows||'<p class="muted">لا توجد عمليات</p>'}</div>`;
    v16PostFormat();
  };
  ...
  window.renderPlayerReport = renderPlayerReport = function(){
    ...
    const b=balances(s)[p]||{}, deps=(s.deposits||[]).filter(d=>d.player===p), games=(s.matches||[]).filter(m=>(m.players||[]).includes(p));
    const playPlusDebt=(b.playTotal||0)+(b.debtDeposits||0);
    const gameRows=games.map(g=>`<div class="tRow"><span>${fmDate(g.date)}</span><span>${escapeHtml(g.place||'')}</span><span class="moneyNeg">${amt(g.price||0,'debt')}</span></div>`).join('');
    const depRows=deps.map(d=>{const k=typeOf(d), c=cls(k);return `<div class="tRow"><span>${fmDate(d.date)}</span><span class="${c}">${label(k)}</span><span class="${c}">${amt(d.amount,k)}</span></div>`}).join('');
    playerFilterContent.innerHTML=`<div class="reportStats"><div class="statBox depositStat"><span>الإيداعات</span><b class="moneyPos">${amt(b.deposits,'deposit')}</b></div><div class="statBox gamesStat"><span>اللعب</span><b>${b.games||''}</b></div><div class="statBox playTotalStat"><span>إجمالي اللعب + المديونية</span><b class="moneyNeg">${amt(playPlusDebt,'debt')}</b></div><div class="statBox lateStat"><span>التأخير</span><b class="moneyLate">${amt(b.late,'late')}</b></div><div class="statBox balanceStat"><span>الرصيد</span><b class="${balClass(b.balance)}">${signed(b.balance)}</b></div></div><div class="card"><h3>أيام اللعب</h3><div class="compactTable reportTable unifiedMoneyTable"><div class="tHead"><span>التاريخ</span><span>المكان</span><span class="amountHead">المبلغ</span></div>${gameRows||'<p class="muted">لا يوجد</p>'}</div></div><div class="card"><h3>الإيداعات والمديونيات</h3><div class="compactTable reportTable unifiedMoneyTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${depRows||'<p class="muted">لا يوجد</p>'}</div></div>`;
    v16PostFormat();
  };

  window.renderAccounts = renderAccounts = function(s){
    const b=balances(s), neg=(s.players||[]).filter(p=>(b[p]?.balance||0)<0), late=(s.deposits||[]).filter(d=>typeOf(d)==='late');
    const extra=(s.extraCharges||[]).reduce((a,x)=>a+Number(x.amount||0),0), discount=(s.extraDiscounts||[]).reduce((a,x)=>a+Number(x.amount||0),0);
    const lateTotal=late.reduce((a,d)=>a+Math.abs(Number(d.amount||0)),0), debt=neg.reduce((a,p)=>a+Math.abs(b[p].balance),0)+discount, final=extra+lateTotal-debt;
    const debtRows=neg.map(p=>`<div class="tRow" onclick="openPlayerReport('${escAttr(p)}')"><span>${escapeHtml(p)}</span><span>${fmDate(b[p].last)}</span><span class="moneyNeg">${amt(Math.abs(b[p].balance),'debt')}</span></div>`).join('');
    const lateRows=late.map(d=>`<div class="tRow" onclick="openPlayerReport('${escAttr(d.player)}')"><span>${escapeHtml(d.player)}</span><span>${fmDate(d.date)}</span><span class="moneyLate">${amt(d.amount,'late')}</span></div>`).join('');
    accountsContent.innerHTML=`<div class="accountMiniCards"><div><span>المديونية</span><b class="moneyNeg">${amt(debt,'debt')}</b></div><div><span>التأخير</span><b class="moneyLate">${amt(lateTotal,'late')}</b></div><div><span>الإضافي</span><b class="moneyPos">${amt(extra,'deposit')}</b></div><div><span>الإجمالي</span><b class="${balClass(final)}">${signed(final)}</b></div></div><div class="card"><h3>اللاعبين المدانين والمتأخرين</h3><div class="compactTable accountsMoneyTable unifiedMoneyTable"><div class="tHead"><span>الاسم</span><span>التاريخ</span><span class="amountHead">المبلغ</span></div>${debtRows}${lateRows}${(!debtRows&&!lateRows)?'<p class="muted">لا يوجد</p>':''}</div></div><div class="card"><div class="sectionTitleLine"><b>سجل الخصم / الإضافة</b><button onclick="openAdjustEditor()">تعديل</button></div>${renderAdjustRows(s)}</div><div class="card"><div class="sectionTitleLine"><b>خصم / إضافة</b></div><div class="grid2 accountsExtraGrid"><label>النوع<select id="extraType"><option value="extra">إضافة</option><option value="discount">خصم</option></select></label><label>التاريخ<button id="extraDateBtn" class="fakeDateInput" type="button" onclick="openDatePicker('extraDate')">${fmDate(today())}</button><input id="extraDate" type="hidden" value="${today()}"></label><label>المبلغ<input id="extraAmount" type="number" step="0.001"></label><label>الملاحظة<input id="extraNote"></label></div><button class="primary wide saveBtn" onclick="saveExtraUnified()">حفظ</button></div>`;
    if(typeof updateDateButtons==='function') updateDateButtons();
    v16PostFormat();
  };
  ...
  window.renderAdjustRows = renderAdjustRows = function(s){
    let rows=[...((s.extraCharges||[]).map(x=>({...x,t:'إضافة',kind:'deposit'}))),...((s.extraDiscounts||[]).map(x=>({...x,t:'خصم',kind:'discount'})))].sort((a,b)=>(b.date||'').localeCompare(a.date||''));
    return `<div class="compactTable accountsMoneyTable unifiedMoneyTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${rows.map(r=>{const c=cls(r.kind);return `<div class="tRow"><span>${fmDate(r.date)}</span><span class="${c}">${r.t}</span><span class="${c}">${amt(r.amount,r.kind)}</span></div>`}).join('')||'<p class="muted">لا يوجد</p>'}</div>`;
  };

  window.v16PostFormat = function(){
    document.querySelectorAll('.moneyPos,.moneyNeg,.moneyLate').forEach(el=>{ el.style.fontWeight='950'; });
    document.querySelectorAll('.compactTable .tRow span, .accountMiniCards b, .reportStats b').forEach(el=>{
      let t=(el.textContent||'').trim();
      if(t==='0.000' || t==='-0.000' || t==='0.000-' || t==='+0.000') el.textContent='';
      // remove plus signs if any old formatter remains
      if(el.textContent) el.textContent=el.textContent.replace(/\+/g,'');
      // move leading minus to the end: -1.000 => 1.000-
      el.textContent=(el.textContent||'').replace(/(^|\s)-(\d+\.\d{3})(?=\s|$)/g,'$1$2-');
      t=(el.textContent||'');
      const hasLate=t.includes('تأخير');
      const hasDebt=t.includes('مديونية')||t.includes('خصم');
      const hasDep=t.includes('إيداع');
      if(hasLate) setClass(el,'late'); else if(hasDebt) setClass(el,'debt'); else if(hasDep) setClass(el,'deposit');
    });
  };
  const oldRenderAll = window.renderAll || renderAll;
  window.renderAll = renderAll = function(){ const r=oldRenderAll.apply(this,arguments); v16PostFormat(); setTimeout(v16PostFormat,0); setTimeout(v16PostFormat,80); return r; };
  setTimeout(()=>{try{renderAll();}catch(e){}},100);
})();
