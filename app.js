...
function restorePlayerNameClicks(){
  const s=state();
  const names=new Set((s.players||[]).map(String));
  document.querySelectorAll('#players .playerItem,#players .playerRow,#players .listRow,#players .checkList label,#players .playerName,#players b,#players span').forEach(el=>{
    const txt=(el.textContent||'').trim();
    if(!txt || !names.has(txt) || el.dataset.reportClick==='1') return;
    el.dataset.reportClick='1';
    el.style.cursor='pointer';
    el.addEventListener('click',ev=>{
      ev.preventDefault();
      ev.stopPropagation();
      openPlayerReport(txt);
    });
  });
}


function dedupeRenamePagesButtons(){
  const btns=[...document.querySelectorAll('button')].filter(b=>(b.textContent||'').trim().includes('إعادة تسمية الصفحات'));
...
function renderPlayers(s){
 playersList.innerHTML=s.players.map(p=>`<button type="button" class="playerChipBtn" onclick="openPlayerReport('${escAttr(p)}');goPage('playerReport');">${escapeHtml(p)}</button>`).join('')||'<p class="muted">أضف اللاعبين أولًا.</p>';
...
function renderAccounts(s){
  const b=balances(s),neg=s.players.filter(p=>(b[p]?.balance||0)<0),late=s.deposits.filter(d=>d.type==='late'),extra=(s.extraCharges.reduce((a,x)=>a+Number(x.amount||0),0))+(s.matches||[]).reduce((a,m)=>a+Number(m.extraFee||0),0),discount=s.extraDiscounts.reduce((a,x)=>a+Number(x.amount||0),0),lateTotal=late.reduce((a,d)=>a+Math.abs(Number(d.amount||0)),0),debt=neg.reduce((a,p)=>a+Math.abs(b[p].balance),0)+discount,final=extra+lateTotal-debt;
  const debtRows=neg.map(p=>`<div class="tRow" onclick="openPlayerReport('${escAttr(p)}');goPage('playerReport')"><span>${escapeHtml(p)}</span><span>${fmDate(b[p].last)}</span><span class="moneyNeg">-${money(Math.abs(b[p].balance))}</span></div>`).join('');
...
(function(){
  ...
  window.renderAccounts = function(s){
    ...
    const extra=((s.extraCharges||[]).reduce((a,x)=>a+val(x.amount),0))+((s.matches||[]).reduce((a,m)=>a+val(m.extraFee||0),0)), discount=(s.extraDiscounts||[]).reduce((a,x)=>a+val(x.amount),0);
    ...
    const debtRows=neg.map(p=>`<div class="tRow" onclick="openPlayerReport('${escAttr(p)}');goPage('playerReport')"><span>${escapeHtml(p)}</span><span>${fmDate(b[p].last)}</span><span class="moneyNeg" data-money-kind="debt">${amt(Math.abs(b[p].balance),'debt')}</span></div>`).join('');
  ...
  window.renderAdjustRows = function(s){
    ...
    return `<div class="compactTable accountsMoneyTable v17MoneyTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${rows.map(r=>{const c=cls(r.kind);const noteVal=r.kind==='matchBonus'?'تكملة القطية':(r.note||'');const amtVal=r.kind==='discount'?amt(r.amount,r.kind)+' -':amt(r.amount,r.kind);return `<div class="tRow${r.kind==='matchBonus'?' readOnlyRow':''}"><span>${fmDate(r.date)}</span><span class="noteCell">${noteVal}</span><span data-money-kind="${r.kind}">${r.t}</span><span class="${c}" data-money-kind="${r.kind}">${amtVal}</span></div>`}).join('')||'<p class="muted">لا يوجد</p>'}</div>`;
  };
...
(function(){
  ...
  try { renderAdjustRows = function(s){
    const matchBonus=(s.matches||[]).filter(m=>Number(m.extraFee||0)>0).map(m=>({id:'mb_'+m.id,date:m.date,amount:Number(m.extraFee),t:'مبلغ إضافي',kind:'matchBonus'}));
    let rows=[...matchBonus,...((s.extraCharges||[]).map(x=>({...x,t:'إضافة',kind:'deposit'}))),...((s.extraDiscounts||[]).map(x=>({...x,t:'خصم',kind:'discount'})))].sort((a,b)=>(b.date||'').localeCompare(a.date||''));
    return `<div class="compactTable accountsMoneyTable"><div class="tHead"><span>التاريخ</span><span>العملية</span><span class="amountHead">المبلغ</span></div>${rows.map(r=>{const c=cls(r.kind);const noteVal=r.kind==='matchBonus'?'تكملة القطية':(r.note||'');const amtVal=r.kind==='discount'?amount(r.amount,r.kind)+' -':amount(r.amount,r.kind);return `<div class="tRow${r.kind==='matchBonus'?' readOnlyRow':''}"><span>${fmDate(r.date)}</span><span class="noteCell">${noteVal}</span><span data-money-kind="${r.kind}">${r.t}</span><span class="${c}" data-money-kind="${r.kind}">${amtVal}</span></div>`}).join('')||'<p class="muted">لا يوجد</p>'}</div>`;
  }; } catch(e) {}
  ...
  try { renderAccounts = function(s){
    ...
    const extra=((s.extraCharges||[]).reduce((a,x)=>a+val(x.amount),0))+((s.matches||[]).reduce((a,m)=>a+val(m.extraFee||0),0)), discount=(s.extraDiscounts||[]).reduce((a,x)=>a+val(x.amount),0);
    const lateTotal=late.reduce((a,d)=>a+Math.abs(val(d.amount)),0), debt=neg.reduce((a,p)=>a+Math.abs(b[p].balance),0)+discount, final=extra+lateTotal-debt;
    const debtRows=neg.map(p=>`<div class="tRow" onclick="openPlayerReport('${escAttr(p)}');goPage('playerReport')"><span>${escapeHtml(p)}</span><span>${fmDate(b[p].last)}</span><span class="moneyNeg" data-money-kind="debt">${amount(Math.abs(b[p].balance),'debt')}</span></div>`).join('');
