...
function renderPlayerTable(s){
 ...
 playerTableWrap.innerHTML=`<div class="tableWrap playersTableWrap"><table class="playersTable finalPlayersTable"><thead><tr><th class="nameTd">الاسم</th><th class="balanceTh">الرصيد</th><th class="gamesTh">لعب</th><th class="lastTh">آخر لعب</th></tr></thead><tbody>${s.players.map(p=>{const bal=Number(b[p]?.balance||0), inactive=isInactiveThisYear(b[p]?.last), isLatest=!!(b[p]?.last&&b[p]?.last===latest);return `<tr class="playerRow ${inactive?'rowInactive':''} ${isLatest?'rowLatest':''}"><td class="nameTd ${inactive?'inactiveName':'normalName'}"><button type="button" class="tableNameBtn ${inactive?'inactivePlayerName':'normalPlayerName'}" onclick="openPlayerReport('${escAttr(p)}')">${escapeHtml(p)}</button></td><td class="balanceTd ${bal<0?'moneyNeg':amountClass(bal)}">${fmAmount(bal)}</td><td class="gamesTd">${b[p]?.games||''}</td><td class="lastTd ${isLatest?'latestDate':'normalDate'}">${fmDate(b[p]?.last)}</td></tr>`}).join('')}</tbody></table></div>`;
}
